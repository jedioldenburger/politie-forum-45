...
try:
    ...
    # Try to load from my-mcp-server directory first (shared .env)
    mcp_env_path = os.path.join(
        os.path.dirname(__file__), "..", "my-mcp-server", ".env"
    )
    if os.path.exists(mcp_env_path):
        load_dotenv(mcp_env_path)
        print(f"✅ Loaded .env from: {mcp_env_path}")
    else:
...
selected_model = "ollama"  # Options: "ollama" (local), "groq" (cloud)
selected_groq_model = "openai/gpt-oss-20b"  # Default: 500k tokens/day, balanced quality (recommended for high volume)
selected_ollama_model = "llama3.2:1b"  # Default Ollama model (fast)
client = None
model_name = None
client_type = None
...
# Available Ollama models (local LAN server)
OLLAMA_MODELS = [
    {
        "name": "llama3.2:1b",
        "description": "Fast, lightweight (1.3B params)",
        "speed": "5-10s/article",
        "priority": 1,
    },  # ⭐ Default - fast
    {
        "name": "bramvanroy/fietje-2b-instruct:Q4_K_M",
        "description": "Dutch-optimized, better quality (2B params)",
        "speed": "10-20s/article",
        "priority": 2,
    },  # 🇳🇱 Best for Dutch
    {
        "name": "bramvanroy/fietje-2b-instruct:Q3_K_M",
        "description": "Dutch-optimized, faster (2B params, quantized)",
        "speed": "8-15s/article",
        "priority": 3,
    },  # 🇳🇱 Balanced Dutch
    {
        "name": "tinyllama:latest",
        "description": "Ultra-fast (1.1B params)",
        "speed": "3-5s/article",
        "priority": 4,
    },  # ⚡ Fastest
]
...
def notify_mcp(article_id, article_data=None, model=None, mode="summary"):
    """
    Notify your local MCP server (Ollama-based) to summarize or analyze a Firestore article.
    MCP server will fetch the article from Firestore /news collection using the article_id.
    Now uses local Ollama models instead of Groq cloud API.
    """
    global selected_ollama_model
    if model is None:
        model = selected_ollama_model
    try:
        params = {"id": article_id, "mode": mode, "force": False, "model": model}
        ...
        res = requests.post(
            "http://localhost:3000", json=payload, timeout=900
        )  # 15 min for large models
        res.raise_for_status()
        print(f"✅ Sent to MCP summarizer: {article_id}")
        result = res.json()
        if "result" in result:
            print("   ↳ MCP optimization complete")
            if "aiSummary" in result.get("result", {}):
                summary_preview = result["result"]["aiSummary"][:100] + "..."
                print(f"   ↳ Summary: {summary_preview}")
        else:
            print("   ↳ MCP response:", result)
    except Exception as e:
        print(f"⚠️ MCP connection failed for {article_id}: {e}")
        print(f"   💡 Make sure MCP server is running on http://localhost:3000")
        print(f"   💡 Start with: cd my-mcp-server && npm start")
...
def set_ollama_model():
    """Set the Ollama AI model for local processing (fetches from Ollama server)"""
    global selected_ollama_model

    # Fetch available models from Ollama server
    print("\n🔍 Ophalen beschikbare modellen van Ollama server...")
    try:
        import requests
        ...
        if response.status_code == 200:
            ...
            if not available_models:
                print("❌ Geen modellen gevonden op Ollama server")
                print("💡 Gebruik standaard lijst...")
                available_models = OLLAMA_MODELS
        else:
            print(
                f"⚠️ Kan geen verbinding maken met Ollama server (HTTP {response.status_code})"
            )
            print("💡 Gebruik standaard lijst...")
            available_models = OLLAMA_MODELS
    except Exception as e:
        print(f"⚠️ Fout bij ophalen modellen: {e}")
        print("💡 Gebruik standaard lijst...")
        available_models = OLLAMA_MODELS

    print("\n🤖 Beschikbare Ollama modellen (Lokaal LAN):")
    print("=" * 80)
    for i, model in enumerate(available_models, 1):
        if isinstance(model, dict) and "size" in model:
            # Live model from server
            dutch_label = "🇳🇱" if "fietje" in model["name"] else ""
            print(f"{i}. {model['name']}")
            print(
                f"   Grootte: {model['size']} | Gewijzigd: {model['modified']} {dutch_label}"
            )
        else:
            # Fallback to static list
            priority_label = "⭐ Aanbevolen" if model.get("priority") == 1 else ""
            dutch_label = "🇳🇱 Nederlands" if "fietje" in model["name"] else ""
            labels = " ".join([priority_label, dutch_label]).strip()
            print(f"{i}. {model['name']}")
            print(f"   {model['description']} | Snelheid: {model['speed']} {labels}")
        print("-" * 80)

    print(f"\n💡 Huidige selectie: {selected_ollama_model}")

    while True:
        choice = input(
            f"\nKies een Ollama model (1-{len(available_models)}, of druk Enter voor huidige): "
        )

        if choice == "":
            print(f"✅ Model blijft: {selected_ollama_model}")
            break

        try:
            choice_int = int(choice)
            if 1 <= choice_int <= len(available_models):
                selected_ollama_model = available_models[choice_int - 1]["name"]
                print(f"✅ Ollama model ingesteld op: {selected_ollama_model}")
                break
            else:
                print("❌ Ongeldige keuze. Probeer opnieuw.")
        except ValueError:
...
def get_ai_client():
    """Get AI client - Uses Groq API (cloud) or falls back to MCP+Ollama (local)"""
    global client, model_name, client_type
    ...
    # Fallback to MCP + Ollama (local)
    print("💡 No Groq API key found - Use Menu Option 26 for MCP+Ollama (local AI)")
    print("   Or set GROQ_API_KEY environment variable for cloud AI")
...
def ai_optimize_news_to_ai_news(db):
    """
    🤖 AI OPTIMIZE: Take articles from /news collection and optimize with MCP Ollama AI.
    Creates optimized versions in /ai_news collection with:
    - AI-generated summary (via local Ollama)
    - Enhanced tags
    - Sentiment analysis
    - Entity extraction
    - Quality scoring
    ...
    """
    print("\n" + "=" * 80)
    print(style("🤖 AI OPTIMIZE - /news → /ai_news (MCP Ollama AI)", CYAN, BOLD))
    print("=" * 80 + "\n")

    print(style("📦 Source: /news collection", DIM))
    print(style("🤖 AI: Local Ollama via MCP (http://localhost:11434)", DIM))
    print(style("💾 Output: /ai_news collection", DIM))
    print(style("✨ Enhancements: tags, summary, sentiment, entities, quality", DIM))
    ...
    try:
        ...
        for article in articles:
            ...
            try:
                # Send to MCP for AI optimization (synchronous)
                print(f"🤖 Sending to MCP for local Ollama AI optimization...")
                notify_mcp(article_id, model=selected_ollama_model, mode="summary")

                # Give MCP time to process and save to Firestore
                import time

                print("⏳ Waiting for MCP to complete processing (5s)...")
                time.sleep(5)

                # Check if MCP successfully saved to /ai_news
                ai_news_doc = ai_news_ref.get()
                if ai_news_doc.exists:
                    ai_data = ai_news_doc.to_dict()
                    print(f"✅ MCP optimization complete")
                    if "aiSummary" in ai_data:
                        print(f"   ✨ AI Summary: {ai_data['aiSummary'][:80]}...")
                    if "aiTags" in ai_data:
                        print(f"   🏷️  AI Tags: {', '.join(ai_data['aiTags'][:5])}")
                    if "aiSentiment" in ai_data:
                        sentiment = ai_data["aiSentiment"]
                        print(
                            f"   🎭 Sentiment: {sentiment.get('sentiment', 'N/A')} ({sentiment.get('score', 0)}/100)"
                        )
                    if "aiQuality" in ai_data:
                        quality = ai_data["aiQuality"]
                        print(
                            f"   ⭐ Quality Score: {quality.get('qualityScore', 0)}/100"
                        )
                    processed += 1
                else:
                    print(f"⚠️  MCP processed but no data in /ai_news yet")
                    skipped += 1
...
def generate_faq(article_text, article_title, max_items=5):
    """
    Generate FAQ items from article content using AI

    Args:
        article_text: Full article body text
        article_title: Article title for context
        max_items: Maximum number of FAQ items (default 5)

    Returns:
        List of FAQ dict items: [{"question": "...", "answer": "..."}]
        Returns empty list if generation fails
    """
    global client, model_name, client_type

    if not client:
        print("⚠️ No AI client available for FAQ generation")
        return []

    try:
        # Strip HTML tags from article text for cleaner analysis
        clean_text = re.sub(r"<[^>]+>", "", article_text)

        # Limit text to first 2000 chars to avoid token limits
        text_preview = clean_text[:2000] if len(clean_text) > 2000 else clean_text

        system_prompt = f"""Je bent een Nederlandse journalist expert in FAQ generatie.
Genereer {max_items} veelgestelde vragen (FAQ) uit dit nieuwsartikel.

CRUCIALE REGELS:
1. Vragen moeten natuurlijk zijn: gebruik wie, wat, waar, wanneer, waarom, hoe
2. Antwoorden EXACT uit artikeltekst (GEEN hallucinatie of aannames)
3. Focus op kernfeiten: data, namen, gevolgen, context
4. Maximaal 2-3 zinnen per antwoord
5. Return ALLEEN valid JSON, geen markdown code blocks

Voorbeelden goede vragen:
- "Wat is er precies gebeurd in [locatie]?"
- "Wie zijn de verdachten in deze zaak?"
- "Wanneer vond het incident plaats?"
- "Waarom gebeurde dit incident?"
- "Hoe reageerde de politie?"

Output formaat (STRICT JSON):
[
  {{"question": "Vraag hier?", "answer": "Antwoord uit artikel."}},
  {{"question": "Tweede vraag?", "answer": "Tweede antwoord."}}
]"""

        user_content = f"""Titel: {article_title}

Artikel:
{text_preview}

Genereer nu {max_items} FAQ items in valid JSON formaat."""

        print(f"   🤖 Generating {max_items} FAQ items with AI...")

        # Call AI based on client type
        if client_type == "anthropic":
        ...
        if not content:
            print("   ⚠️ No content returned from AI")
            return []
        ...
        try:
            ...
            if not isinstance(faq_items, list):
                print("   ⚠️ AI returned non-list JSON")
                return []

            # Validate structure
            valid_faqs = []
            for item in faq_items[:max_items]:
                if isinstance(item, dict) and "question" in item and "answer" in item:
                    # Ensure minimum answer length (avoid one-word answers)
            ...
            print(f"   ✅ Generated {len(valid_faqs)} valid FAQ items")
            return valid_faqs

        except json.JSONDecodeError as e:
            print(f"   ⚠️ Could not parse FAQ JSON: {e}")
...
def advanced_rewriter_v4_groq(db):
    """
    Advanced Rewriter V4 - Groq Cloud AI Workflow

    Python Role (Data Processor):
    - Source: articles_full collection from Firestore
    - AI via Groq Cloud API (gpt-oss-20b or llama-3.3-70b)
    - AI-generated summaries, tags, and content enhancement
    - Structured content generation (H2/H3 hierarchy)
    - FAQ generation (3-5 questions)
    - Location detection with geo-coordinates
    - Save to Firestore /news collection with AI enhancements

    Workflow:
    1. articles_full → Groq AI processing → /news (with AI metadata)
    2. Next.js ISR renders from /news
    """

    try:
        print("\n" + "=" * 80)
        print(style("☁️ ADVANCED REWRITER V4 - Groq Cloud AI", YELLOW, BOLD))
        print("=" * 80)
        print(f"\n📦 Source: articles_full collection")
        print(f"🤖 AI: Groq Cloud ({selected_groq_model})")
        print(f"� Processing: AI-enhanced content generation")
        print(f"💾 Output: Firestore /news (with AI metadata)")
        print(f"🔍 SEO: Enhanced titles, summaries, tags")
        print(f"✨ Features: Cloud AI, high quality, rate limits apply\n")
        ...
        if not client:
            ...
            try:
                ...
                if not client:
                    print(
                        "❌ Groq client initialization failed. Make sure GROQ_API_KEY is set."
                    )
                    print("   Check your .env file for GROK_API_KEY or GROQ_API_KEY")
                    return
        ...
        for doc in articles_docs:
            ...
            try:
                ...
                if article.get("processed_v4_groq", False):
                    print("⏭️  Skipping: Already processed (processed_v4_groq=true)")
                    continue
                ...
                # Clean content
                cleaned_title = clean_text_encoding(raw_title)
                cleaned_body = clean_text_encoding(raw_body)

                # Create slug
                slug = create_url_slug(cleaned_title)

                # AI-generate summary
                print("   📝 Generating AI summary...")
                summary = generate_summary(cleaned_body, char_limit=155)

                # AI-generate optimized title
                print("   🎯 Optimizing title...")
                optimized_title = optimize_title_for_serp(cleaned_title)

                # AI-generate tags
                print("   🏷️  Generating tags...")
                tags = get_tags(cleaned_body)

                # AI-generate category
                print("   📁 Detecting category...")
                category = get_category(cleaned_body)

                # AI-generate FAQ (3-5 questions)
                print("   💬 Generating FAQ items...")
                faq_items = generate_faq(cleaned_body, cleaned_title, max_items=5)

                # Detect location
                location_data = detect_location(cleaned_title, summary, cleaned_body)

                # Generate excerpt
                excerpt = generate_excerpt(cleaned_body, max_words=50)

                # Build article data with AI enhancements
                article_data = {
                    "title": cleaned_title,
                    "titleSeo": optimized_title,
                    "slug": slug,
                    ...
                    "category": category,
                    "tags": tags,
                    "aiTags": tags,  # Same as tags (AI-generated)
                    "faq": faq_items,  # AI-generated FAQ (3-5 items)
                    "location": location_data,
                    "publishedAt": article.get(
                        "published_at", datetime.now().isoformat()
                    ),
                    "author": "DutchJinn Redactie",
                    "source": "Politie.nl",
                    "url": f"https://politie-forum.nl/nieuws/{slug}",
                    "processed_v4_groq": True,
                    "processingMethod": "groq_cloud_v4",
                    "aiModel": selected_groq_model,
                    "createdAt": datetime.now().isoformat(),
                    "optimizedAt": datetime.now().isoformat(),
                }

                # Save to /news collection
                print(f"💾 Saving to /news/{slug}...")
                db.collection("news").document(slug).set(article_data)

                # Mark as processed in articles_full
                db.collection("articles_full").document(article_id).update(
                    {
                        "processed_v4_groq": True,
                ...
                processed += 1
                print(f"✅ Article processed with Groq AI")
        ...
        print(f"\n{'='*80}")
        print(style("🎉 Processing complete!", GREEN, BOLD))
        print(f"✅ Processed: {processed}")
        print(f"❌ Errors: {errors}")
        print(f"{'='*80}\n")
...
def advanced_rewriter_v4_articles_full(db):
    """
    Advanced Rewriter V4 - MCP+Ollama Workflow (100% Local)

    Python Role (Data Processor):
    - Source: articles_full collection from Firestore
    - AI via MCP+Ollama (local, no API keys needed)
    - Structured content generation (H2/H3 hierarchy)
    - FAQ generation (3-5 questions)
    - Location detection with geo-coordinates
    - Save to Firestore /news collection
    - Trigger MCP AI optimization → /ai_news
    ...
    Workflow:
    1. articles_full → Basic processing → /news
    2. MCP+Ollama → AI optimization → /ai_news
    3. Next.js ISR renders from /ai_news
    """

    try:
        print("\n" + "=" * 80)
        print(style("🚀 ADVANCED REWRITER V4 - MCP+Ollama (Local AI)", CYAN, BOLD))
        print("=" * 80)
        print(f"\n📦 Source: articles_full collection")
        print(f"🤖 AI: MCP+Ollama (http://localhost:11434)")
        print(f"� Processing: Basic cleaning + metadata extraction")
        print(f"💾 Output: Firestore /news → MCP AI → /ai_news")
        print(f"🔍 SEO: Next.js ArticleJsonLd.tsx (8+ schemas)")
        print(f"✨ Features: Local AI, no API costs, no rate limits\n")

        # Check MCP server
        try:
            import requests

            res = requests.get("http://localhost:3000/ping", timeout=5)
            if res.text == "pong":
                print("✅ MCP Server is running\n")
            else:
                print("⚠️  MCP Server responded but not healthy\n")
        except:
            print("❌ MCP Server not running! Start with:")
            print("   cd my-mcp-server && npm start\n")
            return
        ...
        for doc in articles_docs:
            ...
            try:
                ...
                # Check if already processed
                if article.get("processed_v4_mcp", False):
                    print("⏭️  Skipping: Already processed (processed_v4_mcp=true)")
                    continue
                ...
                # Basic processing (no AI yet - MCP will handle that)
                print("🧹 Basic cleaning and metadata extraction...")
                ...
                location_data = detect_location(
                    cleaned_title, cleaned_body[:500], cleaned_body
                )

                # Extract basic metadata
                category = article.get("category", "Nieuws")
                tags = article.get("tags", ["Politie", "Nederland"])
                ...
                article_data = {
                    "title": cleaned_title,
                    "titleSeo": optimize_title_for_serp(cleaned_title),
                    "slug": slug,
                    "content": cleaned_body,  # Raw content for MCP to process
                    "excerpt": excerpt,
                    "category": category,
                    "tags": tags,
                    "location": location_data,
                    ...
                    "source": "Politie.nl",
                    "url": f"https://politie-forum.nl/nieuws/{slug}",
                    "processed_v4_mcp": True,
                    "processingMethod": "mcp_ollama_v4",
                    "createdAt": datetime.now().isoformat(),
                }
                ...
                # Trigger MCP AI optimization
                print(f"🤖 Triggering MCP AI optimization...")
                notify_mcp(slug, model=selected_ollama_model, mode="summary")
                ...
                processed += 1
                print(f"✅ Article processed and sent to MCP")
        ...
        print(f"✅ Processed: {processed}")
        print(f"❌ Errors: {errors}")
        print(f"\n💡 Next: Wait 30s for MCP to complete AI optimization")
        print(f"   Then check /ai_news collection in Firestore")
        print(f"{'='*80}\n")
...
def generate_advanced_jsonld(article_data, location_data, faq_data):
    """
    Generate advanced JSON-LD structured data for SEO.

    NOTE: This is now handled by Next.js ArticleJsonLd.tsx component.
    Kept for backward compatibility only.
    """
    # Minimal stub - Next.js handles this
...
def display_menu(show_banner=True, num_articles=1):
    ...
    current_model_display = (
        selected_groq_model if selected_model == "groq" else selected_ollama_model
    )
    model_type_label = "Groq Cloud" if selected_model == "groq" else "Ollama Local"
    ...
    options = [
        ...
        (
            "4",
            f"Kies Groq AI Model (Cloud) (huidig: {style(selected_groq_model, MAGENTA, BOLD)})",
        ),
        (
            "5",
            f"Kies Ollama Model (Lokaal) (huidig: {style(selected_ollama_model, CYAN, BOLD)})",
        ),
        (
            "6",
            f"Wissel AI Provider (huidig: {style(model_type_label, GREEN, BOLD)})",
        ),
        ...
        (
            "25",
            style(
                "✨ ADVANCED REWRITER V4 (articles_full → /news → MCP Ollama AI)",
                GREEN,
                BOLD,
            ),
        ),
        (
            "26",
            style(
                "🤖 AI OPTIMIZE (/news → /ai_news with MCP Ollama - Local AI)",
                MAGENTA,
                BOLD,
            ),
        ),
        (
            "27",
            style(
                "☁️ ADVANCED REWRITER V4 (articles_full → /news with Groq Cloud AI)",
                YELLOW,
                BOLD,
            ),
        ),
        ("28", style("Exit", RED, BOLD)),
    ]
    ...
    while True:
        ...
        if choice == "2":
            ...
            if selected_model == "groq":
                ...
                print(
                    f"✅ AI Provider gewisseld naar: Ollama Local ({selected_ollama_model})"
                )
            else:
                selected_model = "groq"
                print(
                    f"✅ AI Provider gewisseld naar: Groq Cloud ({selected_groq_model})"
                )
        elif choice == "7":
            if rss_url:
